...
and companion .mat files for the Calgary case study.
...
def fig_instance_layout(inst):
    ...
    ax.set_xlabel("x (km, east of Calgary city centre)")
    ax.set_ylabel("y (km, north of Calgary city centre)")
    ax.set_title("Calgary case-study instance layout\n"
...
def fig_patient_demographics(inst):
    ...
    ax.set_title("Patient-type composition -- Calgary case study")
...
def fig_time_window_thresholds(inst):
    ...
    ax.set_title("Semi-soft time-window thresholds -- Calgary case study")
...
def fig_penalty_function(inst):
    ...
    ax.set_title("Semi-soft time-window delay-penalty function -- Calgary case study\n"
...
def fig_disruption_scenario(inst):
    ...
    ax.set_title(f"Realised disruption scenario -- Calgary case study "
...
def fig_qos_vs_gamma():
    ...
    fig.suptitle("Impact of cyber-disruption breadth (Gamma) on quality of service\n"
                  "Calgary case study, Q2GA, 5 seeds; dashed line = default Gamma=0.20")
...
def fig_qos_vs_mttr():
    ...
    ax1.set_title("Recovery time (MTTR) and resulting availability vs. quality of service\n"
                   "Calgary case study, Q2GA, 5 seeds")
...
def fig_availability_surface():
    ...
    ax.scatter([DEFAULT_MTTR], [DEFAULT_MTBF], color="black", marker="x", s=120,
               linewidths=3, label=f"Calgary case-study operating point\n(MTTR={DEFAULT_MTTR:.0f}, MTBF={DEFAULT_MTBF:.0f})")
