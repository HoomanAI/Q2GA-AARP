"""Generate OpenStreetMap-based maps for the Ottawa case study.
...
from src.ottawa_instance import build_instance, HOSPITALS, PATIENTS, latlon_to_xy  # noqa: E402
...
def fig_static_map(inst):
    ...
    ax.set_title("Ottawa case study: hospitals and patient locations\n(OpenStreetMap basemap)")
    ax.set_axis_off()
...
def fig_interactive_map(inst):
    m = folium.Map(location=[45.4215, -75.6972], zoom_start=11, tiles="OpenStreetMap")
...
def fig_route_map(inst):
    ...
    cx.add_basemap(ax, source=cx.providers.OpenStreetMap.Mapnik, crs="EPSG:3857")
    ax.set_title(f"Ottawa case study: best Q2GA route plan\n"
